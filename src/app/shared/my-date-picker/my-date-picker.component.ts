...
export class MyDatePickerComponent implements OnInit, OnChanges {
    ...
    monthLabels: IMyMonthLabels = { 1: 'Jan', 2: 'Feb', 3: 'Mar', 4: 'Apr', 5: 'May', 6: 'Jun',
    ...
    ngOnInit() {
        ...
        let optionprops = ['dayLabels', 'monthLabels', 'dateFormat', 'todayBtnTxt', 'firstDayOfWeek', 'sunHighlight',
            'disableUntil', 'disableSince', 'disableWeekends', 'height', 'width', 'inline'];
    ...
    isDisabledDay(date: IMyDate): boolean {
        ...
        if (this.disableUntil.year !== 0 && this.disableUntil.month !== 0 &&
            this.disableUntil.day !== 0 && givenDate <= this.getTimeInMilliseconds(this.disableUntil)) {
            return true;
        }
        if (this.disableSince.year !== 0 && this.disableSince.month !== 0 &&
            this.disableSince.day !== 0 && givenDate >= this.getTimeInMilliseconds(this.disableSince)) {
            return true;
        }
        ...
        for (let i = 1; i < 7; i++) {
            ...
            if (i === 1) {
                ...
                for (let j = pm; j <= dInPrevM; j++) {
                    ...
                    week.push({dateObj: date, cmo: cmo, currDay: this.isCurrDay(j, m, y, cmo),
                            dayNbr: this.getDayNumber(date), disabled: this.isDisabledDay(date)});
                }
                cmo = this.CURR_MONTH;
                // Current month
                ...
                for (let j = 0; j < daysLeft; j++) {
                    ...
                    week.push({dateObj: date, cmo: cmo, currDay: this.isCurrDay(dayNbr, m, y, cmo),
                            dayNbr: this.getDayNumber(date), disabled: this.isDisabledDay(date)});
                    dayNbr++;
                }
            }else {
    ...
    private _parseDate(ds: string): IMyDate {
        ...
        if (ds !== '') {
            ...
            if (dpos >= 0) {
                rv.day = parseInt(ds.substring(dpos, dpos + 2), 10);
            }
            let mpos = fmt.indexOf('mm');
            if (mpos >= 0) {
                rv.month = parseInt(ds.substring(mpos, mpos + 2), 10);
            }
            let ypos = fmt.indexOf('yyyy');
            if (ypos >= 0) {
                rv.year = parseInt(ds.substring(ypos, ypos + 4), 10);
            }
        }
        return rv;
    }
